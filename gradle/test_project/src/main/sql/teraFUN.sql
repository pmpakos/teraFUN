...
-- Χρόνος δημιουργίας: 22 Φεβ 2018 στις 15:45:04
...
--
DELIMITER $$
CREATE TRIGGER `Ενημέρωση event` AFTER INSERT ON `booking` FOR EACH ROW UPDATE event SET event.TicketCounter = event.TicketCounter + new.NumberOfTickets, event.IncomingCash = event.IncomingCash + (event.Cost * new.NumberOfTickets) WHERE event.EventID = new.EventID
$$
DELIMITER ;
...
INSERT INTO `event` (`EventID`, `CompanyID`, `Name`, `Address`, `DateEvent`, `Hour`, `TicketCounter`, `IncomingCash`, `Cost`, `Description`, `PhotosFolder`, `IsOffer`, `Visits`, `Latitude`, `Longitude`, `MaxCapacity`, `Indoor`, `MinAge`, `MaxAge`, `Fun`, `Sport`, `Education`, `Team`, `TagDescription`) VALUES
(1, 5, 'Πάρτυ Αποκριών', 'Αδριανείου 44, Ελληνορώσων', '2018-02-19', '11:00', 4, 200, 50, 'Ελάτε να γιορτάσουμε τις απόκριες στην myPLAYce. Φορέστε τις πιο δημιουργικές στολές και δηλώστε συμμετοχή στο διαγωνισμό μας! Ο νικητής κερδίζει δωρεάν είσοδο στον παιδότοπο του myPLAYce για έναν μήνα! Μη λείψει κανείς!', 'images/events/event1', 0, 14, '38.00001700000000', '23.77339800000000', 25, 1, 3, 12, 1, 0, 0, 1, 'Παιδότοπος Απόκριες Παιχνίδι Διασκέδαση Φεβρουάριος Παιδί Οικονομικό Διαγωνισμός'),
...
(10, 5, 'Εργαστήριο "Μια Αποκριά, μια ιστορία"', 'Αδριανείου 44, Ελληνορώσων', '2018-04-21', '12:00', 6, 360, 60, 'Διαδραστικό εκπαιδευτικό πρόγραμμα από τη myPlace και τη Δημοτική Βιβλιοθήκη Καλλιθέας \r\n\r\nΗ εταιρεία myPlayce, σε συνεργασία με τη Δημοτική Βιβλιοθήκη Καλλιθέας, παρουσιάζει ένα διαδραστικό εκπαιδευτικό πρόγραμμα, με θέμα τις Απόκριες, το Σάββατο 3 Φεβρουαρίου (ώρα 11:30), στη Δημοτική Βιβλιοθήκη Καλλιθέας (Ανδρομάχης 86Α & Αλκμήνης, Καλλιθέα). \r\n\r\nΤο εκπαιδευτικό πρόγραμμα, που έχει σχεδιάσει η θεατρολόγος Μαρία Νομικού και υλοποιεί η ομάδα θεάτρου Δον Κιχώτες, έχει στόχο να μεταφέρει σε μικρούς και μεγάλους την ιστορία της αποκριάς από την Αρχαία Ελλάδα έως τις μέρες μας.', 'images/events/event10', 0, 11, '38.00001700000000', '23.77339800000000', 15, 1, 6, 12, 1, 0, 1, 1, 'ΠΑΙΔΙΑ ΔΗΜΟΤΙΚΟΥ (6 ΕΩΣ ΚΑΙ 12 ΕΤΩΝ), ΠΑΙΔΙΚΕΣ ΑΠΟΚΡΙΑΤΙΚΕΣ ΕΚΔΗΛΩΣΕΙΣ, ΔΩΡΕΑΝ ΠΑΙΔΙΚΕΣ ΕΚΔΗΛΩΣΕΙΣ');
